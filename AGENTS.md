...
3. BEFORE building, collect the NEW company's brand pack:
   ...
   - ask for: logo (image file or URL), product images (1-3), website link,
...
4. Call `build_journey` and ALWAYS pass:
   ...
   - `industry`, `website`, `logoUrl` (or `logoBase64`), `productImages` from the brand pack
...
6. After each build, report the preview URL from the response (`preview.publicUrl`,
   or `preview.localUrl` if public is null). Open it to verify before telling the
   user the journey is done.
7. After each build, ALSO run `verify_journey` with:
