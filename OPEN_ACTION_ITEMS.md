...
|-----|------|--------|-------|
| P1 | Feature A — content adaptation for new companies | VALIDATED 2026-08-10 | TEST PASSED — hypothesis confirmed: agent with context (reference project + industry knowledge) rewrote all content itself. Evidence: zero Banas text/logo refs (70+20 removed), 37 Meditech refs, 8/8 steps navigate, 44/44 agent verify, audit script all-green, visual pixel probes correct (brand colors, teal on chat screens only). NO rewrite machinery, NO classifier — never build them. Follow-ups: (1) verify_journey MCP tool probes JSON hardening — agent hit single-quote parse error twice, worked around it; (2) WSL-path note for OpenCode agents (copy-to-Windows-workdir pattern works; use UTF8NoBOM); (3) optional medical_equipment industry profile (P3). Test package: whatsapp-mock-generator-mcp/test/content-adaptation-test.md + scripts/audit_content_rewrite.sh. |
