...
const { processAndStore } = require('../../../runtime/uploads/upload-logo');
...
module.exports = (req, res) => {
  ...
  bb.on('finish', async () => {
    ...
    try {
      ...
      if (!sessionId) return res.status(400).end(JSON.stringify({ error: 'sessionId required' }));
      const session = await getSession(sessionId);
      if (!session) return res.status(404).end(JSON.stringify({ error: 'Session not found' }));
      if (!fileBuffer) return res.status(400).end(JSON.stringify({ error: 'No file' }));
