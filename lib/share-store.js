...
const MAX_JOURNEY_BLOB_BYTES = 2.5 * 1024 * 1024; // 2.5 MB per journey — well under Vercel 4.1 MB body limit
...
async function createShare(input, options = {}) {
  ...
  // If a legacy client sends both config and oversized HTML, prefer the tiny config path.
  if (input && input.config && (!input.html || Buffer.byteLength(String(input.html), 'utf8') > MAX_SHARE_HTML_BYTES)) {
    const payload = {
      version: 2,
      token,
      createdAt,
...
async function addJourneyToHub(hubToken, journeyType, html, options = {}) {
  ...
  if (jSize > MAX_JOURNEY_BLOB_BYTES) {
    throw createError(
          'Journey "' + journeyType + '" is too large (' + (jSize / 1024 / 1024).toFixed(1) + ' MB). Max 2.5 MB.',
