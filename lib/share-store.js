...
function journeyBlobPath(hubToken, journeyType) {
  // Sanitize journey type for use as blob key
...
async function createShare(input, options = {}) {
  ...
  if (input && input.config && !input.journeys) {
    const payload = {
      version: 2,
      token,
      createdAt,
  ...
  if (input && input.journeys && Array.isArray(input.journeys)) {
    const blob = getBlobClient(options.blob);
    const journeyTypes = input.journeys.map(function(j) { return j.type; });
    const config = input.config || {};

    // Store each journey HTML as separate blob
    for (const journey of input.journeys) {
      const html = normalizeHtml(journey.html);
      const jSize = Buffer.byteLength(html, 'utf8');
      if (jSize > MAX_JOURNEY_BLOB_BYTES) {
        throw createError(
          'Journey "' + journey.type + '" is too large to share (' + (jSize / 1024 / 1024).toFixed(1) + ' MB). Max is 1.1 MB.',
          'JOURNEY_TOO_LARGE', 413
        );
      }
      await blob.put(journeyBlobPath(token, journey.type), html, {
        access: 'private',
    ...
    const hubPayload = {
      ...
      journeyTypes,
      config,
      journeyBlobs: journeyTypes.map(function(jt) {
        return { type: jt, path: journeyBlobPath(token, jt) };
      })
    };
  ...
  // v1: HTML-based share (backward compatible)
...
async function getShare(token, options = {}) {
  ...
  // v2 config-based shares have no html — caller handles re-rendering
  // v3 multi-blob shares have no html — caller handles hub rendering
