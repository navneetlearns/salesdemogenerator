...
## Long-term Architecture Pivot (July 3, 2026)

**Supabase becomes the single source of truth for content and images.** Managed Postgres + Storage + auto REST API + admin Studio, replacing the LLM-driven content adapter, the per-brand JSON duplication of WhatsApp conversation text, and image storage on disk. One Supabase project serves both this demo-generator repo and the legacy `whatsapp-mock-generator-main` 22-client-project corpus (now being migrated — see `migration/`).

Design spec: `docs/superpowers/specs/2026-07-03-supabase-content-backend-design.md`. Implementation pending user-provisioning a Supabase project (URL + anon key + service-role key in `.env`); the `migration/projects/` workspace, manifest extractor, and image extractor are already committed (`065d623`, `5e19e2d`).
